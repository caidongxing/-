            ...
            <tbody>
               <c:forEach items="${page.result}" var="item">
                <tr>
                 ...
                 <td><span class="label btn-warning">维修中</span></td>
...
function queryPage() {
   $.ajax({
	    cache: true,
        type: "POST",
        url : '${ctx}/roomStatusController/maintainList.do?roomstatus=3',
...
function add(){
  $.post('${ctx}/roomStatusController/maintainroomAddView.do',function(result){
      $('#content').empty();
...
function UpdateView(nohireroomid){
  $.post('${ctx}/roomStatusController/maintainroomUpdateView.do?nohireroomid='+ nohireroomid,function(result){
      layer.open({
      type: 1,
      title: '维修房修改',
      area: '30%',
...
function accomplish(){
    ...
    if(ck.length > 0){
      ...
      layer.confirm('是否要将房间改为可出租状态？', {
        ...
        $.post('${ctx}/roomStatusController/maintainAccomplish.do?ckValue='+ roomsid,
         function(result){
          $('#content').empty();
		  $('#content').append(result);
       });
