	       ...
	       <div class='span12'>
	         <div class='box-content'>
	           <form id="form" class="form-horizontal">
	             ...
	             <input type="hidden" name="roomsid" value="${rooms.roomsid}">
                <div class='control-group'>
                <div class='span6'>
                   <label class='span2' style="line-height: 30px">房号:</label>
                   <input class='span8' name="roomnumber" value="${rooms.roomnumber}" type="number" placeholder="房间数量"/>
                </div>
                ...
                <div class='span6'>
                   ...
                   <option value="">---选择楼号---</option>
                    <c:forEach items="${floors}" var="item">
					 <option value="${item.floorid}" ${item.floorid eq rooms.floorid ? "selected='selected'":""}>${item.floornumber}</option>
					</c:forEach>
				  </select>
                </div>
                </div>
                ...
                <div class='span6'>
                   ...
                   <select class='span8' name="floornumber" id="floornumber">
                    ...
                    <option value="${rooms.floornumber}" ${empty rooms.floornumber ?  "" : "selected='selected'"}>${rooms.floornumber}楼</option>
				   </select>
                </div>
                </div>
               ...
               <div class='span6'>
                   ...
                   <option value="1" ${rooms.roomstatus eq 1 ? "selected='selected'":""}>现住</option>
                   <option value="2" ${rooms.roomstatus eq 2 ? "selected='selected'":""}>空房</option>
                   <option value="3" ${rooms.roomstatus eq 3 ? "selected='selected'":""}>维修房</option>
                   <option value="4" ${rooms.roomstatus eq 4 ? "selected='selected'":""}>非出租房</option>
                   <option value="5" ${rooms.roomstatus eq 5 ? "selected='selected'":""}>锁房</option>
				   </select>
                </div>
                ...
                <div class='span6'>
                   ...
                   <option value="1" ${rooms.orientation eq 1 ? "selected='selected'":""}>东</option>
                   <option value="2" ${rooms.orientation eq 2 ? "selected='selected'":""}>南</option>
                   <option value="3" ${rooms.orientation eq 3 ? "selected='selected'":""}>西</option>
                   <option value="4" ${rooms.orientation eq 4 ? "selected='selected'":""}>北</option>
				   </select>
                </div>
                </div>
                ...
                <a type="button" class="btn btn-primary" onclick="Submit('${ctx}/roomStatusController/update.do')"><i class="icon-save"></i><span>保存</span></a>
